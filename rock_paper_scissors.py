...
class RockPaperScissors:
	...
	def __init__(self):
		...
		# create an empty list to store the all played games, assign the number of played games to 0 and create a list to hold all messages
		self.played_games = []
		self.game_number = 0
		self.messages = []
	...
	def display_greeting_message(self):
		...
		message = self.text_font.render(message_content, True, self.settings.font_color)
	...
	def respond_to_user_input(self):
		"""The user can input rock, paper, scissors and the game receives the input and responds to it"""
	...
	def determine_result(self):
		...
		elif self.user_input == 0 and self.computer_choice == 2 or self.user_input == 2 and self.computer_choice == 1 or self.user_input == 1 and self.computer_choice == 0:
			self.result = 1
	...
	def store_game_data(self):
		"""Stores the game data in dictionaries. The dictionaries are being stored in a list."""
	...
	def create_display_message(self, message_content):
		"""Creates and displays the messages, will be blitted to the screen"""
		message = self.text_font.render(message_content, True, self.settings.font_color)
		self.message_y += self.screen_width * self.settings.pos_y_spacing 
		# Blit the messages to the screen, but only if the messages will be displayed above a cutoff starting from the bottom of the screen
		if self.message_y <= self.screen_height * self.settings.screen_bottom_cutoff:
			# Blit the messages to the screen
			self.screen.blit(message,(self.message_x, self.message_y))

	def create_message_text(self, game):
		# Create the string for the message for the message explaining explaining the game number to the list of messages
		self.messages.append(f'{self.settings.text_game_number} {game["game_number"]+1}')
		# Create the string for the message explaining the user input to the list of messages
		self.messages.append(f'{self.settings.text_user_picked}{self.settings.rock_paper_scissors_mapping[game["user_input"]].title()}')
		# Create the string for the message explaining the computer choice to the list of messages
		self.messages.append(f'{self.settings.text_computer_picked}{self.settings.rock_paper_scissors_mapping[game["computer_choice"]].title()}')
		# Create the string for the message explaining the game result to the list of messages
		self.messages.append(f'{self.settings.text_result[game["result"]]}')
		# Add a spacing between the messages of different games
		self.message_y += self.screen_width * self.settings.pos_y_spacing_between_games
	...
	def _update_screen(self):
		...
		for game in reversed(self.played_games):
			...
			for message in self.messages:
				...
				self.create_display_message(message)
				self.messages = []
		pygame.display.flip()
